...
**Character Generator** offers a hands-on experience in building an interactive character customization application. Users will be able to create and customize characters by selecting various attributes such as hairstyles, eyes, noses, mouths, and accessories. This project encourages creativity and technical skills, providing a comprehensive learning experience in web development.

### What you will learn

- **Dynamic Content Manipulation**: Master the techniques for dynamically updating content on the webpage based on user input, enhancing interactivity and user engagement.

- **Advanced CSS for UI Design**: Dive into advanced CSS to create a visually appealing interface, focusing on layout techniques, transitions, and custom design elements for a unique user experience.

- **JavaScript for Interactive Web Applications**: Learn to use JavaScript for handling user interactions, managing application state, and dynamically updating the DOM to reflect changes in real time.

- **Local Storage for Data Persistence**: Understand how to use the Web Storage API to save and retrieve user selections, ensuring that character customizations persist across browser sessions.

- **Image Processing and Download**: Explore methods for composing images based on user selections and implementing functionality to download the customized character image in various formats.

### Requirements

- Create a centered layout with a "CHARACTER GENERATOR" title. The interface should include a left-side area with a gray background for displaying the character image and a right-side section titled "Customize Look" with options for customization.
- Implement customization options (pills) for hair, eyes, ears, nose, mouth, background, and accessories. Allow users to select one option from each category, displaying a checkmark icon next to the selected option. The character image on the left should update in real-time to reflect selections.
- Include a "Random" button below the character image on the left side. Clicking this button should generate a random character, with the selections reflected by checkmarks next to the corresponding pills on the right side.
- Add a "Download" button to allow users to save their customized character as a JPEG or PNG file, accompanied by a success notification toast indicating the image has been downloaded.
- Utilize local storage to save the user's character customizations. Ensure that the customizations are retained and displayed correctly when the browser is refreshed.
...
2. **Project Page**: Loved the project? Share the [Character Generator](https://app.bigdevsoon.me/projects/character-generator) with your network. It’s a great way to challenge others and see what they can build.
